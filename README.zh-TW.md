...
| 多語系商品內容與索引 | 每個可公開商品都有可直接請求的英文、繁中、簡中網址（`/en/product/:id`、`/zh-tw/product/:id`、`/zh-cn/product/:id`）。頁面由伺服器輸出、各自使用 canonical 並以 `hreflang` 連結；不可交易或無庫存商品不會列入 sitemap。 |
| 公開搜尋頁渲染 | 首頁商品探索區與 `/overview` 第一頁會在伺服器輸出可交易商品；商品更新時會使公開列表、多語商品頁與 sitemap 快取失效。 |
| 搜尋範圍 | 購物車、交易需求、賣家後台、上架與通知等帳號專屬頁面會回傳 `noindex, nofollow`，不作為搜尋落地頁。 |
...
| 站內訊息 | 賣家接受後，買賣雙方可在 Request Center 內傳送純文字訊息；功能由管理者控制的功能開關保護。 |
...
7. 在 Request Center 使用 **訊息** 和賣家協調交易。
8. 也可以建立 **想買清單**，讓系統在有相關新商品時通知你。
...
8. 接受後在 Request Center 的 **訊息** 與買家協調，並確認完成；若交易未完成，系統會恢復庫存。
...
- 站內訊息預設關閉；在 Vercel 啟用前，請先依照[上線指南](docs/trade-messages-rollout.md)完成設定。
...
站內訊息的新增資料表與權限規則請依上線指南執行獨立 migration，完成前不要打開 chat。
