...
| 多语言商品内容与索引 | 每个可公开商品都有可直接请求的英文、繁中、简中网址（`/en/product/:id`、`/zh-tw/product/:id`、`/zh-cn/product/:id`）。页面由服务器输出、各自使用 canonical 并通过 `hreflang` 关联；不可交易或无库存商品不会列入 sitemap。 |
| 公开搜索页渲染 | 首页商品探索区和 `/overview` 第一页会在服务器输出可交易商品；商品更新时会使公开列表、多语言商品页和 sitemap 缓存失效。 |
| 搜索范围 | 购物车、交易请求、卖家后台、发布和通知等账户专属页面会返回 `noindex, nofollow`，不作为搜索落地页。 |
