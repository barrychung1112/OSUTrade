...
- [ ] Add an `after insert` database trigger that emits only `{ requestId, messageId, senderId, createdAt }` as a private Broadcast event. This removes the need for a Vercel function to keep a WebSocket open and prevents message text from appearing in Realtime payloads.
