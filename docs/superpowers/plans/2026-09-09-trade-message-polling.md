...
- [x] Add a fake-timer test that opens a conversation, advances eight seconds, and expects a second message-list request.
- [x] Run the focused test and confirm it fails because no polling timer exists.
- [x] Commit with the component implementation in Task 2.
...
- [x] Add an eight-second `setInterval` after the immediate conversation load.
- [x] Clear the timer when the request changes or the component unmounts.
- [x] Keep existing optimistic send, idempotent retry, read acknowledgement, and visible error handling.
...
- [x] Delete the Realtime RLS policy and insert trigger while retaining message-table RLS and server-only persistence.
- [x] Remove `jose` and the three custom Realtime JWT variables.
- [x] Update rollout instructions so only `TRADE_MESSAGES_ENABLED` gates production chat.
- [x] Update schema assertions to reject Realtime artifacts.
