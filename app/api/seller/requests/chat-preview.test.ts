...
vi.mock("@/app/lib/notifications", () => ({ notifyTradeEvent: vi.fn() }));
...
function query<T>(result: T) {
  ...
  value.eq.mockResolvedValue(result);
  value.in.mockReturnValue(value);
  value.order.mockResolvedValue(result);
  return value;
}

describe("GET /api/seller/requests chat preview", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  test("adds an unread chat preview for an accepted seller request", async () => {
    ...
    const rpc = vi.fn().mockResolvedValue({
      data: [{ request_id: "request-1", unread_count: 1 }],
      error: null,
    });

    mocks.createAdminClient
      .mockReturnValueOnce({ from: vi.fn(() => sellerProductsQuery) })
      .mockReturnValue({
        from: vi.fn(() => requestsQuery),
        rpc,
        auth: {
          admin: {
